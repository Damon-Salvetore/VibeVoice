...
def download_model(model_id: str, model_dir: str = "/app/models") -> str:
    """Download model from HuggingFace to a clean local directory."""
    # Create a clean model name from the model_id
    model_name = model_id.split("/")[-1]  # e.g., "VibeVoice-ASR"
...
Examples:
    ...
    parser.add_argument(
        "--model-dir",
        default="/app/models",
        help="Directory to download model to (default: /app/models)"
    )
    parser.add_argument(
        "--port", "-p",
        type=int,
